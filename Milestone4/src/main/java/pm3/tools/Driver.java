...
public class Driver {
	...
	/**
	 * Test DAO Classes: PlayersDao CharacterInfoDao CharacterEquipmentsDao
	 * EquipmentSlotsDao
	 */
	private static void testPlayerAndCharacters() throws SQLException {
		...
		try {
			...
			CharacterEquipments retrievedEquipment = characterEquipmentsDao
					.getCharacterEquipmentByCharacterAndSlot(newCharacter, mainHandSlot);
	...
	/**
	 * Test DAO Classes: CurrienciesDao CharacterCurrienciesDao
	 */
	private static void testCurrencies() throws SQLException {
		...
		try {
			...
			if (retrievedCurrency != null) {
				System.out.println("Retrieved currency: " + retrievedCurrency.getCurrencyName() + ", Max amount: "
						+ retrievedCurrency.getMaxAmount() + ", Weekly cap: " + retrievedCurrency.getWeeklyCap());
			}
			...
			System.out.println("Updated currency to: " + goldCurrency.getCurrencyName() + ", Max amount: "
					+ goldCurrency.getMaxAmount() + ", Weekly cap: " + goldCurrency.getWeeklyCap());
			...
			CharacterCurrencies retrievedCharacterGold = characterCurrenciesDao
					.getCharacterCurrencyByIDs(character.getCharacterID(), goldCurrency.getCurrencyID());
			if (retrievedCharacterGold != null) {
				System.out.println(
						"Retrieved character currency - Total amount: " + retrievedCharacterGold.getTotalAmountOwned()
	...
	/**
	 * Test DAO Classes: CharacterAttributesDao InventoryPositionsDao
	 */
	private static void testCharacterAttributes() throws SQLException {
		...
		try {
			...
			CharacterAttributes characterAttribute1 = new CharacterAttributes(attribute1, character, 50);
			characterAttribute1 = characterAttributesDao.create(characterAttribute1);

			CharacterAttributes characterAttribute2 = new CharacterAttributes(attribute2, character, 40);
			characterAttribute2 = characterAttributesDao.create(characterAttribute2);

			InventoryPositions inventoryPosition1 = new InventoryPositions(character, 0, item1, 1);
			inventoryPosition1 = inventoryPositionsDao.create(inventoryPosition1);

			InventoryPositions inventoryPosition2 = new InventoryPositions(character, 1, item2, 1);
	...
	/**
	 * Test DAO Classes: ItemsDao EquippableItemsDao ConsumablesDao GearsDao
	 * WeaponsDao ConsumableAttributesDao GearAndWeaponAttributesDao AttributesDao
	 */
	private static void testGearAndWeapon() throws SQLException {
		...
		try {
			...
			ConsumableAttributes potionEffect = new ConsumableAttributes(savedPotion, savedStrength, 100, 0.25f);
			...
			EquippableItems equippableSword = new EquippableItems(savedSword.getItemID(), savedSword.getItemName(),
					savedSword.getMaxStackSize(), savedSword.isMarketAllowed(), savedSword.getVendorPrice(), 1,
					savedMainHandSlot, 1);
			...
			EquippableItems equippableArmor = new EquippableItems(savedArmor.getItemID(), savedArmor.getItemName(),
					savedArmor.getMaxStackSize(), savedArmor.isMarketAllowed(), savedArmor.getVendorPrice(), 2,
					savedArmorSlot, 1);
			...
			GearAndWeaponAttributes swordStrength = new GearAndWeaponAttributes(savedEquippableSword, savedStrength, 5);
			...
			// Create Weapon
			Weapons ironSword = new Weapons(savedEquippableSword.getItemID(), savedEquippableSword.getItemName(),
					savedEquippableSword.getMaxStackSize(), savedEquippableSword.isMarketAllowed(),
					savedEquippableSword.getVendorPrice(), 10, mainHandSlot, 5, 150, 2.5, // autoAttack
					2.8 // attackDelay
			);
			...
			// Create Weapons
			Gears ironArmor = new Gears(savedArmor.getItemID(), savedArmor.getItemName(), savedArmor.getMaxStackSize(),
					savedArmor.isMarketAllowed(), savedArmor.getVendorPrice(), 10, armorSlot, 5, 100, 50);
			...
			ConsumableAttributes retrievedEffect = consumableAttributesDao.getConsumableAttributeByItemIdAndAttributeId(
					savedPotion.getItemID(), savedStrength.getAttributeID());
	...
	/**
	 * Test DAO Classes: JobsDao CharacterJobsDao
	 */
	private static void testJobs() throws SQLException {
		...
		try {
			...
			for (Jobs job : jobsDao.getAllJobs()) {
				System.out.println("Job: " + job.getJobName() + " (ID: " + job.getJobID() + ")");
			}
			...
			System.out.println(
					"Deleted character-job relationship for Character " + char2Rogue.getCharacter().getCharacterID());
